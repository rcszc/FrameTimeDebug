...
#define IMCB_CHARTMEP_BYTES 1024
	...
	void ListDrawRoundImage(const ImVec2& position, const ImVec2& size, ImTextureID texture) {
		...
		ImGui::GetWindowDrawList()->AddImageRounded(
			...
			ImVec2(0.0f, 0.0f),
			ImVec2(1.0f, 1.0f),
			IMVEC4_TO_COLU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)),
			ImGui::GetStyle().FrameRounding
		);
	}
	...
	namespace RzAnim {
#define FLOAT_SIZE_CLAMP(size) ImVec2(IM_MATHS_CLAMP((size).x, 0.0f, FLT_MAX), IM_MATHS_CLAMP((size).y, 0.0f, FLT_MAX))

		ImVec4 FLOAT_COLOR_CLAMP(const ImVec4& color) {
			return ImVec4(
				IM_MATHS_CLAMP(color.x, 0.0f, 1.0f), IM_MATHS_CLAMP(color.y, 0.0f, 1.0f),
				IM_MATHS_CLAMP(color.z, 0.0f, 1.0f), IM_MATHS_CLAMP(color.w, 0.0f, 1.0f)
			);
		}
